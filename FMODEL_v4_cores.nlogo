...
turtles-own [ real_x real_y step_lengths core_count ]
...
to go
  ...
  ;;create new turtle, place in random location with carry_in cores
  ...
  crt 1 [
    ...
    set real_x xcor
    set real_y ycor
    set core_count carry_in
    set step_lengths []
  ]

  ;;if the agent is still in the window of observation
  ;;;;make cores if there are none and move to the next location
...
;;MAKE CORES PROCEDURE
to make-cores
  ;;add 1 core to the agents' kit, and add the number of flakes removed from
  ;;the core to the local assemblage
 repeat overproduce [
...
to step
  ...
  ;;if the agent is still in the window of observation, discard a core
  if real_x <= (max-pxcor + 0.5) and real_x >= (min-pxcor - 0.5) and real_y <= (max-pycor + 0.5) and real_y >= (min-pycor - 0.5) and core_count > 0 [
...
@#$#@#$#@
<experiments>
  <experiment name="Flake Target 100" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    ...
    <steppedValueSet variable="carry_in" first="0" step="0.1" last="1"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
