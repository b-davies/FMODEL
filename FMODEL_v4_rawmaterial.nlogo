...
;;SETUP VARIABLES
globals [ turtle_count exp_csa obs_csa full_cortex cortex_ratio_left cortex_ratio_right move_count reduction_count]
...
to setup
  ...
  ; setup abundances of raw material patches on left and right sides
  ask n-of (left_abundance * count patches with [ pxcor < x-lim ] ) patches with [ pxcor < x-lim ] [
    set pcolor red
  ]

  ask n-of (right_abundance * count patches with [ pxcor >= x-lim ] ) patches with [ pxcor >= x-lim ] [
    set pcolor red
  ]

crt 1 [  ; create an agent
    setxy random-xcor random-ycor
    set step_lengths []
    set color green
    set size 2
  ]
  set reduction_count 0
  reset-ticks
end
...
to go
  ...
  if reduction_count >= 100 and [ flake_count ] of turtle 0 <= 0 [
    get-cr
    stop
  ]

  ;;make flakes if there are none and the agent is on a raw material patch
  ;;then move to the next location
  ask turtles [
    if flake_count <= 0 and [ pcolor ] of patch-here = red [
      make-flakes
      set reduction_count reduction_count + 1
    ]
    step
...
to step
  ...
  let step-length 500001
  while [ step-length > 500000 ] [

    ifelse xcor < x-lim [
      set step-length (random-float 1.000) ^ (-1 / left_mu )
    ]
    [
      set step-length (random-float 1.000) ^ (-1 / right_mu )
    ]
  ]
  ...
  ;;if the agent has flakes, discard a flake
  if  flake_count > 0 [
    set artefact_type lput 1 artefact_type
...
to get-cr
  ...
  ;;if there are any artefacts in the left section
  ifelse sum [ length artefact_type ] of patches with [pxcor < x-lim]  > 0 [

    ;;get number of cores, number of flakes left on cores, and number of flakes
    let assem_cores (sum [ length filter[ [?1] -> ?1 = 0 ] artefact_type ] of patches with [pxcor < x-lim])
    let assem_flakes_on_cores assem_cores * (20 -( reduction *  20))
    let assem_flakes (sum [ length filter[ [?1] -> ?1 = 1 ] artefact_type ] of patches with [pxcor < x-lim])
    ...
    set cortex_ratio_left obs_csa / exp_csa
  ]
  [
    set cortex_ratio_left 99999
  ]
  ...
  ifelse sum [ length artefact_type ] of patches with [pxcor >= x-lim]  > 0 [
    ...
    set exp_csa nod_csa * mod_num_cob
    set obs_csa ((assem_flakes + assem_flakes_on_cores) / 20) * nod_csa
    set cortex_ratio_right obs_csa / exp_csa
  ]
  [
    set cortex_ratio_right 99999
  ]

  ;;same for whole area
  ifelse sum [ length artefact_type ] of patches  > 0 [
    ...
    set exp_csa nod_csa * mod_num_cob
    set obs_csa ((assem_flakes + assem_flakes_on_cores) / 20) * nod_csa
    set full_cortex obs_csa / exp_csa
  ]
  [
    set full_cortex 99999
  ]
...
355
Left Flakes
sum [ length filter [ [?1] -> ?1 = 1 ] artefact_type ] of patches with [ length artefact_type > 0 and pxcor < x-lim]
17
1
11
...
403
Left Cores
sum [ length filter [ [?1] -> ?1 = 0 ] artefact_type ] of patches with [ length artefact_type > 0 and pxcor < x-lim]
17
1
11
...
@#$#@#$#@
<experiments>
  <experiment name="Raw Material 100" repetitions="100" runMetricsEveryStep="false">
    <setup>setup</setup>
    <go>go</go>
    <metric>cortex_ratio_left</metric>
    <metric>cortex_ratio_right</metric>
    <enumeratedValueSet variable="x-lim">
      <value value="16"/>
      <value value="32"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="left_mu">
      <value value="1"/>
      <value value="3"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="left_abundance">
      <value value="0.1"/>
      <value value="0.9"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="right_mu">
      <value value="1"/>
      <value value="3"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="right_abundance">
      <value value="0.1"/>
      <value value="0.9"/>
    </enumeratedValueSet>
  </experiment>
